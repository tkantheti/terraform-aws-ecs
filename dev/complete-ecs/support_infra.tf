...
module "vpc" {
  ...
  #private_subnets = var.private_subnets
   ...
   public_subnets = [for i in range(length(local.vpc_azs)) : cidrsubnet(local.vpc_cidr, 2, i)]
  # private_subnets = [for i in range(length(local.vpc_azs)) : cidrsubnet(local.vpc_cidr, 2, i+2)]
